...
# Available Paths
    def get_paths(self, src, dst):
        ...
        print("=== Running Multipath DFS Modification algorithm ===")
        print("Iterasi : ", i)
        #print("All paths from sw ", src, " to ", dst, " : ", paths)
        paths.sort(key=len)
        #print("sorted paths by length from sw ", src, " to ", dst, " : ", paths)
...
# Installing paths        
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        for path in paths:
            ...
            print("Result available paths from:", src, " to ",dst, " :", path, "cost = ", pw[len(pw) -1 ])

        # saving to csv
        header = ['timestamp', 'Available paths from : ', 'to : ', 'path : ', 'cost : ', 'Path installation finished in :']
        data = [datetime.now(), src, dst, path, pw, time.time() - computation_start]

        with open('./data_export/multipath_dfs_modif.csv', 'a+', newline='') as f:
    ...
    def link_delete_handler(self, ev):
        ...
        for i in self.paths:
            if s1.dpid in list(i[0][0].keys()) and s2.dpid in list(i[0][0].keys()):
